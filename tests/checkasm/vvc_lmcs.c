...
static void check_vvc_lmcs_16bpc_8pixels(void)
{
    ...
    if(check_func(c.lmcs.filter[1], "ff_16bpc_8_pixels_test"))
    {
        for(int ctu_size_idx = 0; ctu_size_idx < 1; ++ctu_size_idx)
        {
            ...
            {
                call_new(src1 + i, MAX_CTU_SIZE*2, ctu_sizes[ctu_size_idx], 8, lut0);
                call_ref(src0 + i, MAX_CTU_SIZE*2, ctu_sizes[ctu_size_idx], 8, lut0);
            }
            if(memcmp(src0, src1, MAX_CTU_SIZE * MAX_CTU_SIZE * 2 ))
            {
                fail();
            }
            bench_new(src0, MAX_CTU_SIZE*2, ctu_sizes[ctu_size_idx], 8, lut0);
        }
    }
    report("16bpc_8pixels");
}



static void check_vvc_lmcs_16bpc_16pixels(void)
{
    static const int ctu_width = 16;
    const int bit_depth = 10;
    VVCDSPContext c;
    ff_vvc_dsp_init(&c, bit_depth);
    ...
    if(check_func(c.lmcs.filter[2], "ff_16bpc_%d_pixels", ctu_width))
    {
        call_new(src1, MAX_CTU_SIZE*2, ctu_width, ctu_width, lut0);
        call_ref(src0, MAX_CTU_SIZE*2, ctu_width, ctu_width, lut0);
        if(memcmp(src0, src1, MAX_CTU_SIZE * MAX_CTU_SIZE * 2 ))
        {
            fail();
        }
        bench_new(src1, MAX_CTU_SIZE*2, ctu_width, ctu_width, lut0);
    }
    report("lmcs_16bpc_16_pixels");
}

static void check_vvc_lmcs_16bpc_32pixels(void)
{
    static const int ctu_width = 32;
    const int bit_depth = 10;
    VVCDSPContext c;
    ff_vvc_dsp_init(&c, bit_depth);
    ...
    report("lmcs_16bpc_32_pixels");
}

static void check_vvc_lmcs_16bpc_64pixels(void)
{
    static const int ctu_width = 64;
    const int bit_depth = 10;
    VVCDSPContext c;
    ff_vvc_dsp_init(&c, bit_depth);
    ...
    report("lmcs_16bpc_64_pixels");
}

static void check_vvc_lmcs_16bpc_128pixels(void)
{
    const int bit_depth = 10;
    VVCDSPContext c;
    ff_vvc_dsp_init(&c, bit_depth);
    ...
    static const int ctu_width = 128;
    ...
    report("lmcs_16bpc_128_pixels");
}

static void check_vvc_lmcs_16bpc_fuzz(void)
{
    const int bit_depth = 10;
    VVCDSPContext c;
    ff_vvc_dsp_init(&c, bit_depth);
...
void checkasm_check_vvc_lmcs(void)
{
    ...
    check_vvc_lmcs_16bpc_16pixels();
    check_vvc_lmcs_16bpc_32pixels();
    check_vvc_lmcs_16bpc_64pixels();
    check_vvc_lmcs_16bpc_128pixels();
    check_vvc_lmcs_16bpc_fuzz();
}
