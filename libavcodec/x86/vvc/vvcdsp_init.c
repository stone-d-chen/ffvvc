...
void ff_lmcs_8bpc_16pix_avx2(uint8_t *_dst, ptrdiff_t dst_stride, const int width, const int height, const uint8_t *_lut);
...
void ff_vvc_dsp_init_x86(VVCDSPContext *const c, const int bd)
{
    ...
    if (EXTERNAL_AVX2(cpu_flags)) {
        switch (bd) {
            case 8:
                AVG_INIT(8, avx2);
                c->lmcs.filter[2] = ff_lmcs_8bpc_16pix_avx2; // 16
