    ...
    if (ciip_flag) {
        ...
        if (sc->sh.r->sh_lmcs_used_flag) {
            if(sbw == 4) {
                ...
                fc->vvcdsp.lmcs.filter[1](inter, inter_stride, sbw, sbh, fc->ps.lmcs.fwd_lut);
            } else {
                fc->vvcdsp.lmcs.filter[2](inter, inter_stride, sbw, sbh, fc->ps.lmcs.fwd_lut);
            }
        }
...
static void predict_inter(VVCLocalContext *lc)
{
    ...
    if (lc->sc->sh.r->sh_lmcs_used_flag && !cu->ciip_flag) {
        ...
        if(cu->cb_width >= 16) {
            fc->vvcdsp.lmcs.filter[2](dst0, fc->frame->linesize[LUMA], cu->cb_width, cu->cb_height, fc->ps.lmcs.fwd_lut);
            ...
            fc->vvcdsp.lmcs.filter[1](dst0, fc->frame->linesize[LUMA], cu->cb_width, cu->cb_height, fc->ps.lmcs.fwd_lut);
        }
    }
}
