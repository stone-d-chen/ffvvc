    ...
    const AVFrame *ref0, const AVFrame *ref1, const int x_off, const int y_off, const int block_w, const int block_h)
{
    ...
    *orig_mv = *mvf;
    ...
    if(block_w == block_h)
    {
        min_sad = fc->vvcdsp.inter.sad[av_log2(block_h) - 2](tmp[L0], tmp[L1], dx, dy, block_w, block_h);
    }
    else
    {
        min_sad = fc->vvcdsp.inter.sad[0](tmp[L0], tmp[L1], dx, dy, block_w, block_h);
    }
    min_sad -= min_sad >> 2;
    sad[dy][dx] = min_sad;
        ...
        for (dy = 0; dy < SAD_ARRAY_SIZE; dy++) {
            for (dx = 0; dx < SAD_ARRAY_SIZE; dx++) {
                if (dx != sr_range || dy != sr_range) {
                    if(block_w == block_h)
                    {
                        sad[dy][dx] = fc->vvcdsp.inter.sad[av_log2(block_h) - 2](lc->tmp, lc->tmp1, dx, dy, block_w, block_h);
                    }
                    else
                    {
                        sad[dy][dx] = fc->vvcdsp.inter.sad[0](lc->tmp, lc->tmp1, dx, dy, block_w, block_h);
                    }
