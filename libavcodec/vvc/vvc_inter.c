    ...
    if (ciip_flag) {
        ...
        if (sc->sh.r->sh_lmcs_used_flag) {
            if(sbw == 4) {
                ...
                fc->vvcdsp.lmcs.filter[3](inter, inter_stride, sbw, sbh, fc->ps.lmcs.fwd_lut.u8);
            }
        }
...
static void predict_inter(VVCLocalContext *lc)
{
    ...
    if (lc->sc->sh.r->sh_lmcs_used_flag && !cu->ciip_flag) {
        ...
        if(cu->cb_width == 16) {
            ...
            fc->vvcdsp.lmcs.filter[1](dst0, fc->frame->linesize[LUMA], cu->cb_width, cu->cb_height, fc->ps.lmcs.fwd_lut.u8);
        } else {
            fc->vvcdsp.lmcs.filter[3](dst0, fc->frame->linesize[LUMA], cu->cb_width, cu->cb_height, fc->ps.lmcs.fwd_lut.u8);
        }
    }
}
